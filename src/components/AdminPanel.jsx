...
import { GROUP_LABELS, buildKnockoutStages, getCountryFlagUrl } from '../lib/tournamentUtils';
...
export default function AdminPanel() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const [{ data: teamsData, error: teamsError }, { data: allMatchesData, error: allMatchesError }, { data: scheduledMatchesData, error: scheduledMatchesError }] = await Promise.all([
    ...
    const handleGenerateFixtures = async () => {
        ...
        const groupedTeams = GROUP_LABELS.map((label) => teams.filter((team) => String(team.group_id || '').toUpperCase() === label));
        ...
        if (allMatches.length > 0) {
            const shouldContinue = window.confirm('Fixtures already exist. Do you want to regenerate them?');
    ...
    const handleGenerateKnockoutFixtures = async () => {
        ...
        const completedMatches = allMatches.filter((match) => {
            const homeScoreIsSet = match.home_score !== null && match.home_score !== undefined;
            const awayScoreIsSet = match.away_score !== null && match.away_score !== undefined;
            return (
        ...
        const hasAllGroupResults = allMatches.length >= 24 && completedMatches.length >= 24;

        if (!hasAllGroupResults) {
            alert('Please finalize all group-stage matches before generating knockout fixtures.');
            return;
        }

        const { quarterfinals, semifinals, final } = buildKnockoutStages(teams, allMatches);
    ...
    return (
        <div className="admin-panel">
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
                <button className="view-toggle-btn" onClick={() => setActiveAdminTab('teams')} style={{ opacity: activeAdminTab === 'teams' ? 1 : 0.7 }}>
                    Teams
                </button>
                <button className="view-toggle-btn" onClick={() => setActiveAdminTab('fixtures')} style={{ opacity: activeAdminTab === 'fixtures' ? 1 : 0.7 }}>
                    Fixtures
                </button>
                <button className="view-toggle-btn" onClick={() => setActiveAdminTab('results')} style={{ opacity: activeAdminTab === 'results' ? 1 : 0.7 }}>
                    Results
                </button>
            </div>
            ...
            {activeAdminTab === 'teams' && (
                <>
                    <section style={{ marginBottom: '1.5rem' }}>
                        <h3>Add Team</h3>
                        <form onSubmit={handleAddTeam} style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                            <input
                                type="text"
                                value={newTeamName}
                        ...
                        <div style={{ display: 'grid', gap: '0.5rem' }}>
                            {GROUP_LABELS.map((group) => (
                                <div key={group}>
                                    ...
                                    {teams.filter((team) => String(team.group_id || '').toUpperCase() === group).map((team) => {
                                        const teamName = team.name || team.team_name;
                                        const flagUrl = getCountryFlagUrl(teamName);
                                        return (
                                            <span key={team.id} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.35rem', marginRight: '0.5rem' }}>
                                                {flagUrl ? <img src={flagUrl} alt={`${teamName} flag`} className="country-flag" /> : null}
                ...
                <section style={{ marginBottom: '1.5rem' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                        <button onClick={handleGenerateFixtures} disabled={submitting}>
                            {submitting ? 'Generating Fixtures...' : 'Generate Group Fixtures'}
                        </button>
                        <button onClick={handleGenerateKnockoutFixtures} disabled={submitting}>
                            {submitting ? 'Generating Knockout Fixtures...' : 'Generate Knockout Fixtures'}
                        </button>
                    </div>
                </section>
            )}
                ...
                <>
                    {!selectedMatch ? (
                        <div className="match-selector">
                            ...
                            {matches.length === 0 ? (
                                ...
                                <ul className="pending-matches-list">
                                    {matches.map((m, index) => {
                                        const home = teams.find((team) => team.id === m.home_team_id);
                                        const away = teams.find((team) => team.id === m.away_team_id);
                                        ...
                                        return (
                                            <li key={m.id}>
                                                <button onClick={() => handleSelectMatch(m)}>
                                                    Match {index + 1}: {homeFlag ? <img src={homeFlag} alt={`${homeName} flag`} className="country-flag" /> : null} {homeName} vs {awayFlag ? <img src={awayFlag} alt={`${awayName} flag`} className="country-flag" /> : null} {awayName}
                                                </button>
                                            </li>
                                        );
                                    })}
                        ...
                        <div className="match-entry-form">
                            <h3>
                                Finalizing: {teams.find((team) => team.id === selectedMatch.home_team_id)?.name || teams.find((team) => team.id === selectedMatch.home_team_id)?.team_name} vs {teams.find((team) => team.id === selectedMatch.away_team_id)?.name || teams.find((team) => team.id === selectedMatch.away_team_id)?.team_name}
                            </h3>
                            <button className="back-btn" onClick={() => setSelectedMatch(null)}>← Back to List</button>

                            <form onSubmit={handleSubmit} className="score-form">
                                <div className="team-column">
                                    <h4>{teams.find((team) => team.id === selectedMatch.home_team_id)?.name || teams.find((team) => team.id === selectedMatch.home_team_id)?.team_name} (Home)</h4>
                                ...
                                <div className="team-column">
                                    <h4>{teams.find((team) => team.id === selectedMatch.away_team_id)?.name || teams.find((team) => team.id === selectedMatch.away_team_id)?.team_name} (Away)</h4>
